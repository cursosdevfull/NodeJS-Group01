        ...
        if (schemas[it]) {
          ...
          switch (it) {
            case 'body':
              ...
              resultValidation = schemas[it].validate(req.body);
              break;
            case 'params':
              listValidations.push(schemas[it].validate(req.params));
              resultValidation = schemas[it].validate(req.params);
              break;
            case 'query':
              listValidations.push(schemas[it].validate(req.query));
              resultValidation = schemas[it].validate(req.query);
              break;
            case 'headers':
              listValidations.push(schemas[it].validate(req.headers));
              resultValidation = schemas[it].validate(req.headers);
              break;
          }
        }
      });
